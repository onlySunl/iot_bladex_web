        ...
        <el-col :span="24">
          <basic-container>
            <el-collapse v-model="logActiveNames" @change="handleChange">
              <el-collapse-item
                title="4.9.0.RELEASE发布，新增全栈AI开发Skills集合，新增视图机制，新增认证日志系统"
                name="45"
              >
                <div>1.[升级]SpringBoot 至 3.5.13</div>
                <div>2.[升级]SpringCloud 至 2025.0.2</div>
                <div>3.[升级]Nacos 至 3.1.2</div>
                <div>
                  4.[新增]BladeX-Skills全栈开发AI技能集合，为Claude Code提供框架专属开发辅助能力
                </div>
                <div>
                  5.[新增]BladeX-Skills涵盖头脑风暴、规范驱动开发、轻量规划、全栈代码生成、代码审计、跨工程对比与同步、知识问答、Git提交等完整工作流
                </div>
                <div>6.[新增]BladeView视图序列化机制，支持按角色动态控制JSON输出字段</div>
                <div>7.[新增]@PermitAll全局认证放行注解，支持声明式控制身份认证与权限认证</div>
                <div>8.[新增]UserTokenHandler支持自定义Token解析逻辑</div>
                <div>9.[新增]API Key调用日志记录与角色解析机制，支持异步持久化</div>
                <div>10.[新增]认证日志模块，记录用户登录认证信息</div>
                <div>11.[新增]认证锁定模块，重构用户锁定判定逻辑</div>
                <div>
                  12.[新增]MybatisPlusInterceptorCustomizer拦截器定制器，支持自定义扩展拦截器链
                </div>
                <div>13.[新增]接口权限与数据权限列表树形双视图模式及抽屉配置功能</div>
                <div>
                  14.[新增]角色管理、顶部菜单、租户产品包的权限树节点联动开关与全选反选功能
                </div>
                <div>15.[新增]用户管理角色配置行级操作入口与节点联动全选反选功能</div>
                <div>16.[新增]顶部菜单主页动态指定功能与全量菜单切换能力</div>
                <div>17.[新增]多级路由支持keep-alive跨层级缓存</div>
                <div>18.[重构]租户模块的删除、回收、恢复逻辑</div>
                <div>19.[优化]Secure安全拦截体系，增强ClientInterceptor路径排除与PermitAll支持</div>
                <div>20.[优化]RedisCacheWriter批量策略，使用SCAN替代KEYS避免性能风险</div>
                <div>
                  21.[优化]OAuth2Request参数构建流程，统一HttpRequest获取避免重复ThreadLocal访问
                </div>
                <div>22.[优化]API Key认证查询策略，拆分联表为分步单表查询</div>
                <div>23.[优化]Feign传递令牌的顺序与逻辑</div>
                <div>24.[优化]动态数据源加载驱动类逻辑</div>
                <div>25.[优化]WebUtil新增getUserAgent请求参数重载</div>
                <div>26.[优化]超管操作OSS配置逻辑</div>
                <div>
                  27.[优化]采用新版findColumn方法替换findObject方法，适配不同场景兼容性
                </div>
                <div>
                  28.[修复]BladePermissionHandler权限列表toList()返回不可变集合导致缓存反序列化异常的问题
                </div>
                <div>29.[修复]角色单行配置权限获取不到角色ID的问题</div>
                <div>30.[修复]部分浏览器关于表格高度的兼容性问题</div>
              </el-collapse-item>
              <el-collapse-item
              ...
              <el-collapse-item
                title="4.6.0.RELEASE发布，新增KingBase兼容支持，新增数据审计模块，新增接口防抖"
                name="42"
              >
