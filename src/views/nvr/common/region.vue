      ...
      <splitpanes class="default-theme custom-splitpanes">
        <pane size="20" min-size="15">
          <div class="tree-card">
            <div class="tree-header">
              ...
              <span class="tree-title">行政区划</span>
            </div>
        ...
        <pane size="80">
          <div class="main-content">
            <!-- 行政区划信息卡片 -->
            <div class="region-info-card" :class="{ 'has-region': civilCodeName }">
              <div class="region-info-content">
                <el-icon class="region-info-icon"><MapLocation /></el-icon>
                <div class="region-info-text">
                  <span class="region-info-label">所选行政区划</span>
                  <el-text v-if="civilCodeName" type="primary" class="region-name">{{ civilCodeName }}</el-text>
                  <el-text v-else type="warning" class="region-name empty">未选择行政区划</el-text>
                </div>
              </div>
              <div class="region-info-badge" v-if="civilCodeName">
  ...
  methods: {
    ...
    handleNodeClick(data) {
      if (data.deviceId == null || data.deviceId == undefined) {
        ...
        if (data.deviceId != null || data.deviceId != undefined) {
          this.queryParams.gbCivilCode = data.deviceId;
          this.addDisabled = false;
        } else {
          this.queryParams.gbCivilCode = null;
          this.addDisabled = true;
        }
        this.civilCodeName = data.name;
        this.regionDeviceId = this.queryParams.gbCivilCode;
        handleQuery();
    },
    handleSelectionChange(selection) {
      if (this.queryParams.gbCivilCode === null) {
          this.multiple = true;
        } else {
    ...
    handleAdd() {
      if (this.regionDeviceId === "" || this.regionDeviceId === " ") {
    ...
    getListDevice() {
      this.loadingSelect = true;
        queryListByCivilCode(this.queryParamsSelect)
          .then((res) => {
    ...
    handleSelect(row) {
      if (row.id) {
          addDeviceToCivilCode(this.regionDeviceId, [row]);
        } else {
          addDeviceToCivilCode(this.regionDeviceId, this.selectionSelectList);
        }
        proxy.$modal.msgSuccess("选择成功");
        this.open = false;
    },
    addDeviceToCivilCode(regionDeviceId, data) {
      let deviceIds = [];
