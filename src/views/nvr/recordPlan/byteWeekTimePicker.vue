...
const handleMousedown = (event) => {
  startIndex = event.target.getAttribute('data-index');
  if (disableTimes.includes(~~startIndex)) return;
  isMove = true;
  axis.startx = startIndex % DayTimes;
  axis.starty = ~~(startIndex / DayTimes);
};
...
const handleMousemove = (event) => {
  ...
  axis.endx = index % DayTimes;
  axis.endy = ~~(index / DayTimes);
  preViewIndex = getSelectIndex();
};
...
const getSelectIndex = () => {
  ...
  return indexList.filter(v => !disableTimes.includes(v));
};

const setSelectIndex = (indexList) => {
  if (!Array.isArray(indexList)) return;
  let listLength = indexList.length;
  let newData = list[startIndex] === '1' ? '0' : '1';
  for (let i = 0; i < listLength; i++) {
    list.splice(indexList[i], 1, newData);
  }
  const inputValue = list.join('');
...
const showSelectTime = (list) => {
  ...
  weeksSelect.forEach((item) => {
    showTimeText.push(getTimeText(item));
  });
};
...
const initList = (value) => {
  ...
  if (value && reg.test(value)) {
    list = value.split('');
    return showSelectTime(list);
  }
  list = new Array(weekTimes).fill('0');
