 ...
 * @returns 颜色值
 */
export const getStorageColor = (used, total) => {
 ...
 * @returns HTTP URL
 */
export const convertWsToHttp = (wsUrl) => {
  if (!wsUrl) return '';
 ...
 * @returns 配置类型名称
 */
export const getConfigTypeName = (key) => {
 ...
 * @returns 字段名称
 */
export const getConfigFieldName = (configType, fieldKey) => {
 ...
 * @returns 是否在线
 */
export const isDeviceOnline = (status | string) => {
 ...
 * @returns 状态类型
 */
export const getDeviceStatusType = (status | string) => {
 ...
 * @returns 防抖后的函数
 */
export const debounce = <T extends (...args[]) => any>(fn: T, delay = 300) => {
 ...
 * @returns 节流后的函数
 */
export const throttle = <T extends (...args[]) => any>(fn: T, interval = 300) => {
  let lastTime = 0;
  return function(this, ...args: Parameters<T>) {
    const now = Date.now();
