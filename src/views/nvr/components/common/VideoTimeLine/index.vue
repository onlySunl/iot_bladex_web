<template>
  <div
      ...
      :class="{ 'is-dark': resolvedColors.isDark }"
      :style="{
...
export default {
  ...
  props: {
    ...
    */
    ...
    lineHeightRatio: {
      ...
      default() {
        return {
          date: 0.35, // 0点时的日期线段高度
          time: 0.22, // 显示时间的线段高度
          none: 0.1, // 不显示时间的线段高度
          hover: 0.32 // 鼠标滑过时显示的时间段高度
        }
      }
    },
    // 鼠标滑过时是否显示实时所在的时间
  ...
  methods: {
    ...
    drawMiddleLine() {
      ...
      const {width} = this.centerLineStyle
      const color = this.resolvedColors.centerLineColor
      ...
      this.ctx.save()
      this.ctx.beginPath()
      this.ctx.moveTo(x - pointerSize, 0)
      this.ctx.lineTo(x + pointerSize, 0)
      this.ctx.lineTo(x, pointerSize + 1)
      this.ctx.closePath()
    ...
    addGraduations() {
      ...
      // 修复：重新计算起始偏移，确保对齐到正确的刻度点
      const firstVisibleTime = this.startTimestamp
      let firstGridTime
      
      // 找到第一个不早于startTimestamp的网格时间
      const remainder = firstVisibleTime % msPerGrid
      if (remainder === 0) {
        firstGridTime = firstVisibleTime
      } else {
        firstGridTime = firstVisibleTime - remainder + msPerGrid
      }
      
      const msOffset = firstGridTime - this.startTimestamp
      const pxOffset = (msOffset / msPerGrid) * pxPerGrid
      
      for (let i = 0; i < gridNum; i++) {
        const currentStartTimestamp = firstGridTime + i * msPerGrid
        ...
        let h = 0
        let lineWidth = 1
        // 修复：创建date时使用更加精确的方式，避免舍入误差
        const date = new Date(Math.round(graduationTime / 1000) * 1000)
        // 0点显示日期
        if (this.showDateAtZero && date.getHours() === 0 && date.getMinutes() === 0) {
          h = this.height * (this.lineHeightRatio.date === undefined ? 0.35 : this.lineHeightRatio.date)
          lineWidth = 2
          this.ctx.fillStyle = this.resolvedColors.textStrong
          ...
          // 其余时间根据各自规则显示
          h = this.height * (this.lineHeightRatio.time === undefined ? 0.22 : this.lineHeightRatio.time)
          this.ctx.fillStyle = this.resolvedColors.textColor
        ...
        this.drawLine(x, 0, x, h, lineWidth, this.resolvedColors.lineColor)
      }
    },
    ...
    checkShowTime(date) {
      ...
      // 修复：为了避免由于时间精度问题导致错过12点，我们做一个容错检查
      // 检查原始date附近的时间点
      const rule = this.ACT_ZOOM_DATE_SHOW_RULE[this.currentZoomIndex]
      ...
      // 然后检查前后一秒的时间，防止精度问题导致错过
      const time = date.getTime()
      for (let offset = -1000; offset <= 1000; offset += 1000) {
        if (offset !== 0) {
          const nearDate = new Date(time + offset)
          if (rule(nearDate)) {
            return true
          }
        }
    ...
    drawTimeSegments(callback, path) {
      ...
      this.timeSegments.forEach((item) => {
        if (
          ...
          const radius = Math.min(4, segH / 2, segW / 2)
          ...
          if (path) {
            ...
            // 渐变填充
            const gradient = this.ctx.createLinearGradient(segX, segY, segX, segY + segH)
    ...
    hoverShow(x, noDraw) {
      ...
      const h = this.height * (this.lineHeightRatio.hover === undefined ? 0.32 : this.lineHeightRatio.hover)
      this.drawLine(x, 0, x, h, 1, this.resolvedColors.lineColor)
      ...
      this.ctx.fillStyle = this.resolvedColors.bgPanel
      this.drawRoundRect(this.ctx, bgX, bgY, bgW, bgH, 4)
      this.ctx.fill()

      // 绘制文字
      this.ctx.fillStyle = this.resolvedColors.hoverTextColor
      this.ctx.textAlign = 'center'
      this.ctx.fillText(t, x, bgY + 17)
    ...
    graduationTitle(datetime) {
      // 修复：先把datetime取整，确保和checkShowTime使用的是同一个时间
      const normalizedTime = new Date(Math.round(datetime / 1000) * 1000)
    ...
    syncThemeColors() {
      ...
      const primary = this.getCssVar('--el-color-primary', '#409eff')
      const textPrimary = this.getCssVar('--el-text-color-primary', isDark ? '#ffffff' : '#303133')
      const textRegular = this.getCssVar('--el-text-color-regular', isDark ? '#e5e7eb' : '#606266')
      const borderColor = this.getCssVar('--el-border-color', isDark ? '#4c4d4f' : '#dcdfe6')

      this.resolvedColors.centerLineColor = primary
      this.resolvedColors.isDark = isDark

      if (isDark) {
        // 暗黑模式 - 使用更亮更明显的颜色
        this.resolvedColors.bgPanel = 'rgba(0, 0, 0, 0.85)'
        this.resolvedColors.textStrong = '#ffffff'
        this.resolvedColors.textColor = '#e5e7eb'
        ...
        this.resolvedColors.hoverTextColor = '#ffffff'
        this.resolvedColors.lineColor = 'rgba(255,255,255,0.3)'
      }
    },
      ...
      const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
          if (mutation.attributeName === 'class') {
            const isDark = document.documentElement.classList.contains('dark')
            if (this.resolvedColors.isDark !== isDark) {
              this.syncThemeColors()
              this.reRender()
            }
          }
...
.timeLineContainer.is-dark .windowList {
