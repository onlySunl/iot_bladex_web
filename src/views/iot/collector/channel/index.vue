  ...
  <basic-container>
    ...
    >
      <template slot-scope="{ row, type, disabled }" slot="menuForm">
        <modbus-config-form ref="formRef" :form-data="form" :type="type" :disabled="disabled" @submit="handleFormSubmit" />
      </template>
    </avue-crud>
  </basic-container>
</template>
...
<script>
import { getList, getDetail, add, update, remove } from '@/api/iot/collector/modbus-config'
import ModbusConfigForm from './modbus-config-form'

export default {
  name: 'ModbusConfigPage',
  components: { ModbusConfigForm },
  data() {
    return {
      form: {},
      tableData: [],
