  ...
  <basic-container>
    ...
    >
      <template slot-scope="{ row, type, disabled }" slot="menuForm">
        <notify-template-form ref="formRef" :form-data="form" :type="type" :disabled="disabled" @submit="handleFormSubmit" />
      </template>
    </avue-crud>
  </basic-container>
</template>
...
<script>
import { getList, getDetail, add, update, remove } from '@/api/iot/notice/alert-notify-template'
import NotifyTemplateForm from './notify-template-form'

export default {
  name: 'AlertNotifyTemplatePage',
  components: { NotifyTemplateForm },
  data() {
    return {
      form: {},
      tableData: [],
