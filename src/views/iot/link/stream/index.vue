  ...
  <basic-container>
    ...
    >
      <template slot-scope="{ row, type, disabled }" slot="menuForm">
        <sip-server-form ref="formRef" :form-data="form" :type="type" :disabled="disabled" @submit="handleFormSubmit" />
      </template>
    </avue-crud>
  </basic-container>
</template>
...
<script>
import { getList, getDetail, add, update, remove } from '@/api/iot/link/sip-server'
import SipServerForm from './sip-server-form'

export default {
  name: 'SipServerPage',
  components: { SipServerForm },
  data() {
    return {
      form: {},
      tableData: [],
