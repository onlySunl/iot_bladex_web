  ...
  <basic-container>
    ...
    >
      <template slot-scope="{ row, type, disabled }" slot="menuForm">
        <mqtt-client-form ref="formRef" :form-data="form" :type="type" :disabled="disabled" @submit="handleFormSubmit" />
      </template>
    </avue-crud>
  </basic-container>
</template>
...
<script>
import { getList, getDetail, add, update, remove } from '@/api/iot/link/mqtt-client'
import MqttClientForm from './mqtt-client-form'

export default {
  name: 'MqttClientPage',
  components: { MqttClientForm },
  data() {
    return {
      form: {},
      tableData: [],
